...
//修改函数4
static void HumanClassifyAiProcess(VIDEO_FRAME_INFO_S frm, VO_LAYER voLayer, VO_CHN voChn)
{
    int ret;
    if (GetCfgBool("human_classify_switch:support_human_classify", true)) {
        if (g_workPlug.model == 0) {
            ret = Yolo3HandDetectResnetClassifyLoad(&g_workPlug.model);
            if (ret < 0) {
                g_workPlug.model = 0;
                SAMPLE_CHECK_EXPR_GOTO(ret < 0, HAND_RELEASE,
                    "load human classify model err, ret=%#x\n", ret);
            }
        }

        VIDEO_FRAME_INFO_S resizeFrm;
        ret = MppFrmResize(&frm, &resizeFrm, HAND_FRM_WIDTH, HAND_FRM_HEIGHT);
        ret = Yolo3HandDetectResnetClassifyCal(g_workPlug.model, &resizeFrm, &frm);
        SAMPLE_CHECK_EXPR_GOTO(ret < 0, HAND_RELEASE,
            "human classify plug cal FAIL, ret=%#x\n", ret);
...
//修改函数3
static HI_VOID* GetVpssChnFrameHumanClassify(HI_VOID* arg)
{
    int ret;
    VIDEO_FRAME_INFO_S frm;
    HI_S32 s32MilliSec = 2000;
    ...
    while (HI_FALSE == g_bAiProcessStopSignal) {
        ...
        //修改函数4
        HumanClassifyAiProcess(frm, voLayer, voChn);
    }
...
//修改函数1
static HI_S32 HumanClassifyAiThreadProcess(HI_VOID)
{
    HI_S32 s32Ret;
    if (snprintf_s(acThreadName, BUFFER_SIZE, BUFFER_SIZE - 1, "AIProcess") < 0) {
        HI_ASSERT(0);
    ...
    prctl(PR_SET_NAME, (unsigned long)acThreadName, 0, 0, 0);
    //修改函数3
    s32Ret = pthread_create(&g_aiProcessThread, NULL, GetVpssChnFrameHumanClassify, NULL);
...
//修改函数2
static HI_S32 PauseDoUnloadHumanClassifyModel(HI_VOID)
{
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    if (GetCfgBool("human_classify_switch:support_human_classify", true)) {
        ...
        /* when pause operation, should unload model */
        s32Ret = Yolo3HandDetectResnetClassifyUnload(g_workPlug.model);
        SAMPLE_CHECK_EXPR_RET(s32Ret != HI_SUCCESS, s32Ret, "unload human model err:%x\n", s32Ret);
        ConfBaseExt();
        g_num = 0;
    }
...
//修改函数0
HI_S32 SAMPLE_MEDIA_HUMAN_CLASSIFY(HI_VOID)
{
    HI_S32             s32Ret;
    HI_S32             fd = 0;
    ...
    /* create work thread to run ai */
    //修改函数1
    s32Ret = HumanClassifyAiThreadProcess();
    SAMPLE_CHECK_EXPR_RET(s32Ret != HI_SUCCESS, s32Ret, "ai proccess thread creat fail:%s\n", strerror(s32Ret));
    ...
    g_aiProcessThread = 0;
    //修改函数2
    PauseDoUnloadHumanClassifyModel();
